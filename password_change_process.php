...
if ($res_bc_code==false)
		...
		$count_user_table = mysql_num_rows($res_bc_code);
		$result_bc_code = mysql_fetch_array($res_bc_code);
		$cspCode = $result_bc_code['username'];
		if($count_user_table >0)
		  {
		  $check_bc_code_in_user = "select * from tbl_bcmaster where Cspcode = '".$bc_code."' AND date_of_birth = '".$new_date_of_birth."' "; 
		...
		if($count_in_user ==0)
		{
		  header("Location:forgot_password.php?msg=date_of_birth_not_matched");
		}
		else{
										...
										<div class="header_left_back"></div>
											<div class="header_middle_back1">&nbsp;
											  <a href="index.php"><img src="images/my_home.png" align="absmiddle"></a>&nbsp;
												<span class="heading_text" >Working Capital Enhancement Forgot Password</span></div>
								...
								<td  valign="top">
								 <table width="93%" border="0" cellspacing="3" cellpadding="3"  align="center" />
								   ...
								   <fieldset id="fieldset3" class="fieldset">
									<legend class="form_legend_heading_maroon">Reset Password</legend>
									<table width="50%" border="0" cellspacing="3" cellpadding="3"  align="center" class="data" >
									<form id="myForm" method="POST" action="forgot_password_process.php" onsubmit="return checkForm(this);">
										<tr>
...
<?php
     ...
     header("Location:forgot_password.php?msg=user_not_available");
}
?>